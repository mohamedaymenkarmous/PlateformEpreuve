...
public function getTasksAction() {
 try {
	$chaine_brute = file_get_contents( $this->container->getParameter('epreuvesFile') );
 ...
 $rep = $this->getDoctrine()->getManager()->getRepository("SecurinetsFrontOfficeBundle:Epreuve");
 $rep2 = $this->getDoctrine()->getManager()->getRepository("SecurinetsFrontOfficeBundle:EpreuveChallenger");

 try {
	$epreuves = $rep->getByListId($list);
	$validatedTasks = $rep2->findTaskByUser(
		$this->container->get("security.token_storage")->getToken()->getUser()->getId()
	);
 } catch (DBALException $ex) {
	$logger = $this->get('monolog.logger.dbalException');
	$logger->warn( $this->getLogMessage("getValidatedTasks (getTasksAction)") );
	$epreuves = array();
 }
 return $this->render("SecurinetsFrontOfficeBundle:Challenge:tasks.html.twig",array(
		"epreuves" => $epreuves,
...
public function getTaskAction($id_epreuve) {
 ...
 try {
	...
	$rep = $manager->getRepository("SecurinetsFrontOfficeBundle:Epreuve");
	$rep1 = $manager->getRepository("SecurinetsFrontOfficeBundle:EpreuveChallenger");
	$epreuve = $rep->find($id_epreuve);
	$x = $rep1->findByIdUserAndIdEpreuve(
		$this->container->get("security.token_storage")->getToken()->getUser()->getId(),
		$id_epreuve
	);
	if ($x != null)$solved = true;
 ...
 return $this->render("SecurinetsFrontOfficeBundle:Challenge:task.html.twig",array(
		'epreuve' => $epreuve,
		'solved' => $solved)
	);
}
...
public function validateAction($id,Request $request) {
 ...
 try {
	$user = $manager->getRepository("SecurinetsUsersBundle:User")->find($id_user);
 } catch (DBALException $ex) {
	$logger = $this->get('monolog.logger.dbalException');
 ...
 if ($session->has($id)) {
	...
	if ($diff < 2){ // 5 secondes avant de revalidé
		...
		try {
			$manager->flush();
		} catch (DBALException $ex) {//logger DBALException
			$logger = $this->get('monolog.logger.dbalException');
			$logger->warn( $this->getLogMessage("banned  (validateAction)") );
		}

		//logger Banned member
		$logger = $this->get('monolog.logger.bannedMember');
		$logger->info( $this->getLogMessage("is banned  (validationAction)") );
		return new Response(json_encode($reponse));//tester sur l'ip
	}
 }
 $session->set($id, time());
 ...
 if ($id == 0) {
	...
	$logger->info( $this->getLogMessage("\$id=0 (url modifie)  (validationAction)") );
 ...
 if (!$request->isXmlHttpRequest()){ // requet qui vienne d'une autre place
	$logger = $this->get('monolog.logger.erreurDeclenche');
	$logger->info( $this->getLogMessage("validation !isXmlHttpRequest  (validationAction)") );
 ...
 $ec = $manager->getRepository("SecurinetsFrontOfficeBundle:EpreuveChallenger")->findByIdUserAndIdEpreuve($id_user, $id);
 if ($ec != null) {
 ...
 $repository = $manager->getRepository("SecurinetsFrontOfficeBundle:Epreuve");
 try {
 ...
 if ($rightFlag == $flag) {
	...
	$user->setScore($score)->setLastValidate(time());
	...
	$ec->setEpreuve($epreuve)->setUser($user)->setTime(new \DateTime("NOW"));
	...
	try {
		...
		//write into log
		$logger = $this->get('monolog.logger.dbalException');
		$logger->warn( $this->getLogMessage("can't register the validation  (validationAction)") );
	}

	$logger = $this->get('monolog.logger.validation');
	$logger->info( $this->getLogMessage("validate task ".$epreuve->getName() . "(validationAction)") );
	$reponse['result'] = true;
	return new Response(json_encode($reponse));
 }
...
public function getScoreAction() {
 ...
 //var_dump($user);
 try {
	$list = $rep->getScore(null);
 } catch (DBALException $ex) {
	//write into log
	$logger = $this->get('monolog.logger.dbalException');
	$logger->warn( $this->getLogMessage("can't recieve the score  (scoreAction)"));
 }

 return $this->render("SecurinetsFrontOfficeBundle:Challenge:rating.html.twig",array(
		'list' => $list
	));
}
...
public function getStatAction() {
 ...
 try {
	$list = $rep->getStat();
	//$epreuves = $rep->getByListId($list);
 //	$validatedTasks = $rep2->findTaskByUser( $this->container->get("security.token_storage")->getToken()->getUser()->getId());
 } catch (DBALException $ex) {
	//write into log
	$logger = $this->get('monolog.logger.dbalException');
	$logger->warn( $this->getLogMessage("can't recieve the score  (scoreAction)") );
 ...
 return $this->render("SecurinetsFrontOfficeBundle:Challenge:stat.html.twig",array(
		'list' => $list
	));
}
...
public function getRankAction() {
 ...
 $rep = $this->getDoctrine()->getManager()->getRepository("SecurinetsUsersBundle:User");
 try {
	$rank = $rep->getRank($id);
 } catch (DBALException $e) {
	$logger = $this->get('monolog.logger.dbalException');
	$logger->warn( $this->getLogMessage("can't get the rank (rankAction)") );
	$rank = 0;
 }
...
public function challengerInfoAction(){
 ...
 if ($this->get("security.authorization_checker")->isGranted('ROLE_CHALLENGER')) {
	...
	//$rep = $em->getRepository("SecurinetsUsersBundle:User");
	//var_dump($rep->_em()/*->get('repositoryFactory')*/);
	try {
		$rank = $rep->getRank($id);
	} catch (DBALException $e) {
		$logger = $this->get('monolog.logger.dbalException');
...
/*public function validateFlagAction(Request $request){
 ...
 try{
	...
	if ($flag != null){
		...
		$vf->setEquipe($user)->setFlag($flag);
	...
	$logger = $this->get('monolog.logger.dbalException');
	$logger->warn( $this->getLogMessage("Validation flag problem".$e->getMessage()) );
 }
 return new Response(json_encode($reponse));
}*/
